# python tools/fetch_workorders_v6_singley_jobs.py
...
for job_id in wo_ids:
    url = f"{BASE_URL}/smwe/api/v6/workorders/jobs/{job_id}"
    try:
        response = requests.get(url, headers=headers)
        print(f"Status Code: {response.status_code} | job_id: {job_id} | URL: {url}")
        detail_path = os.path.join(output_dir, f"{job_id}.json")
        ...
        print(f"Exception for job_id {job_id}: {e}")
