...
# --- SQL VARCHAR/NVARCHAR/CHAR/NCHAR columns and max lengths from your schema ---
...
def trim_all_varchar_cols(df, col_max_lengths):
    print(f"Trimming columns to max lengths: {col_max_lengths}")
    for col, maxlen in col_max_lengths.items():
    ...
    for col, maxlen in col_max_lengths.items():
        if col in df.columns and pd.api.types.is_string_dtype(df[col]):
            ...
            if still_over.any():
                print(f"WARNING: Still over limit in column '{col}'. Number of offending rows: {still_over.sum()}")
    return df
...
def main():
    for filename, table_name in ordered_table_files:
        ...
        col_max_lengths = TS_TABLE_VARCHAR_LENGTHS.get(table_name, {})
        print(f"Using SQL-defined max lengths for {table_name}: {col_max_lengths}")
        df = trim_all_varchar_cols(df, col_max_lengths)
