#  python tools/vintrace_Bulk_Stock_Report_playwright_test.py
...
REPORT_CONFIG = {
    ...
    "second_report_filename": "Vessel_Contents.csv",
...
SECOND_FILE_PATH = os.path.join(TARGET_SAVE_DIR, REPORT_CONFIG["second_report_filename"])
...
async def run():
    ...
    try:
        async with async_playwright() as p:
            ...
            # Step 6: Select Vintage year robustly
            try:
                ...
                result = await page.select_option(f'#{REPORT_CONFIG["vintage_dropdown_id"]}', label=REPORT_CONFIG["vintage_option_text"])
                print(f"Vintage select result: {result}")
                if not result or '2025' not in result:
                    print("2025 not found or not selected!")
                else:
                    print("Vintage 2025 selected.")
            except Exception as e:
                print(f"Could not select vintage dropdown option: {e}")

            # Step 7: Click "Composition details" checkbox (first checkbox)
            try:
            ...
            # Step 9: Click second checkbox (after dropdowns)
            try:
            ...
            try:
                ...
                # Save as both filenames:
                for target_path in [TARGET_FILE_PATH, SECOND_FILE_PATH]:
                    try:
                        if os.path.exists(target_path):
                            os.remove(target_path)
                            print(f"Deleted old file: {target_path}")
                        shutil.copy(temp_path, target_path)
                        print(f"Copied downloaded report to: {target_path}")
                    except Exception as file_err:
                        print(f"Failed to replace {target_path}: {file_err}")
                # Always remove temp file
                try:
                    os.remove(temp_path)
                except Exception as temp_err:
                    print(f"Could not remove temp file: {temp_err}")
            except Exception as e:
                print(f"Could not click the Generate button or download file: {e}")

            # Optional: Pause browser for inspection
            print("\nAll steps complete. The browser window will remain open for inspection and manual action.\n")
            await page.pause()

    finally:
        print(f"Report should be downloaded and named: {TARGET_FILE_PATH} and {SECOND_FILE_PATH}")
