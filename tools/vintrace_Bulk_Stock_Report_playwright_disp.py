...
ZERO_RECORDS_PATH = os.path.join(TARGET_SAVE_DIR, "zero_records_info.json")  # NEW
...
def load_zero_record_files(zero_records_path):
    # NEW: Returns a set of filenames that had zero records
    if not os.path.exists(zero_records_path):
        return set()
    try:
        with open(zero_records_path, "r", encoding="utf-8") as zr:
            zero_data = json.load(zr)
            return set(entry.get("file") for entry in zero_data if "file" in entry)
    except Exception as e:
...
async def run():
    ...
    report_inputs = load_csv_inputs(CSV_INPUT_PATH, limit_rows=200)   #-------------------------------------------
    ...
    zero_record_files = load_zero_record_files(ZERO_RECORDS_PATH)   # NEW
    print(f"Files with zero records (will be skipped): {zero_record_files}")
    ...
    try:
        async with async_playwright() as p:
            ...
            # --- Main Loop: Run for first 300 CSV rows ---
            ...
            for idx, params in enumerate(report_inputs):
                ...
                # Skip if already processed or if in zero_record_files
                if file_base in already_processed or os.path.exists(target_path):
                    print(f"Skipping already processed report: {file_base}")
                    continue
                if file_base in zero_record_files:
                    print(f"Skipping zero-record file: {file_base}")
                    continue
