#  python tools/vintrace_playwright_dispatch_search_console_fix_partials.py
import asyncio
import os
import sys
import datetime
...
def normalize_date(date_str):
    """Convert M/D/YYYY or MM/DD/YYYY to MM-DD-YYYY."""
...
def load_redownload_records(csv_path):
...
async def fill_dispatch_search_dates_and_bol(page: Page, date: str, bol: str):
    # Convert date from MM-DD-YYYY to MM/DD/YYYY for UI fields
    date_for_ui = datetime.datetime.strptime(date, "%m-%d-%Y").strftime("%m/%d/%Y")
    print(f"Setting From date: {date_for_ui} and To date: {date_for_ui} and BOL: {bol}")
    try:
        await page.wait_for_selector('#c_353', timeout=LARGE_TIMEOUT)
        await page.fill('#c_353', date_for_ui)
        print(f'Filled From date: {date_for_ui}')
    except Exception as e:
        print(f"Could not fill From date: {e}")
    try:
        await page.wait_for_selector('#c_363', timeout=LARGE_TIMEOUT)
        await page.fill('#c_363', date_for_ui)
        print(f'Filled To date: {date_for_ui}')
    except Exception as e:
...
async def select_dispatch_table_row_by_key(page: Page, key_tuple):
    ...
    for i, row in enumerate(rows):
        ...
        if len(cells) > 2:
            ...
            try:
                date = datetime.datetime.strptime(date_raw, "%m/%d/%Y").strftime("%m-%d-%Y")
            except Exception:
                date = date_raw.replace("/", "-")
            bol = (await cells[1].inner_text()).strip()
            quantity = (await cells[2].inner_text()).strip()
            all_row_keys.append((date, bol, quantity))
    print(f"Available table rows: {all_row_keys}")
    print(f"Trying to match key_tuple: {key_tuple}")
...
async def process_redownload_records(page: Page, records):
    for rec in records:
        print(f"\n--- Processing for Date: {rec['date']} BOL: {rec['bol']} Qty: {rec['quantity']} ---")
        try:
            await fill_dispatch_search_dates_and_bol(page, rec['date'], rec['bol'])
            await click_dispatch_search_button(page)
            await page.wait_for_timeout(1000)
            await page.wait_for_selector("#c_305_tbody > tr", timeout=LARGE_TIMEOUT)
            key_tuple = (rec['date'], rec['bol'], rec['quantity'])
            info = await select_dispatch_table_row_by_key(page, key_tuple)
            if not info:
                print(f"Skipping {rec}: could not select or gather info.")
                continue
            if not await click_view_wine_details(page):
                print(f"Skipping {rec}: could not open wine details.")
                continue
            if not await click_fruit_tab(page):
                print(f"Skipping {rec}: could not open Fruit tab.")
                continue
...
async def main():
    ...
    REDOWNLOAD_LIST_CSV = "Main/data/vintrace_reports/disp_console/missing_dispatches.csv"  # Use the missing ones CSV

    if not USERNAME or not PASSWORD:
        print("VINTRACE_USER or VINTRACE_PW environment variables not set.")
        sys.exit(1)

    records = load_redownload_records(REDOWNLOAD_LIST_CSV)
    ...
    async with async_playwright() as p:
        ...
        # Step 3: For each missing record in the CSV, search and process
        await process_redownload_records(page, records)

        print("All specified records have been processed. Script complete.")
        await browser.close()
