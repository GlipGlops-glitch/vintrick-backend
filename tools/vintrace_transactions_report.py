#  python tools/vintrace_Grape_Report.py
import os
import shutil
...
TARGET_SAVE_DIR = r"C:\Users\cah01\OneDrive - SMWE\Documents - Winetracking-Data\VintTrick"
...
try:
    ...
    # Step 6: Click on Operations tab/menu
    try:
        # Try direct by ID
        vintage = wait.until(EC.element_to_be_clickable((By.ID, "c_385|Text")))
        vintage.click()
        print("Clicked 'Operations' by ID.")
    except Exception as e:
        print(f"Could not click Operations by ID: {e}")
        # Fallback: Try by visible text
        try:
            vintage = wait.until(EC.element_to_be_clickable((By.XPATH, "//span[text()='Operations']")))
            vintage.click()
            print("Clicked 'Operations' by text.")
        except Exception as e2:
            print(f"Could not click Operations by text: {e2}")
            # Fallback: Try clicking parent if span is not clickable
            try:
                vintage_span = wait.until(EC.presence_of_element_located((By.ID, "c_385|Text")))
                parent = vintage_span.find_element(By.XPATH, "..")
                wait.until(EC.element_to_be_clickable(parent)).click()
                print("Clicked parent of 'Operations' span.")
            except Exception as e3:
                print(f"Could not click parent of Operations span: {e3}")
    ...
    # Step 9: Click the "Generate..." button to run the report
    try:
    ...
    # Step 10: Find the latest downloaded file with the VINx2_DeliveryReport_2025* pattern and rename/replace grape_delivery_report.csv
    try:
