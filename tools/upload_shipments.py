...
# Now with CHUNKED/batch upload for large files!
...
CHUNK_SIZE = int(os.getenv("CHUNK_SIZE", "500"))  # Number of rows per batch
...
def flat_row_generator(json_path):
        ...
        for wine_detail in wine_details:
            ...
            if compositions:
                for composition in compositions:
                    yield extract_flat_row(s, wine_detail, composition)
            else:
                yield extract_flat_row(s, wine_detail, None)
...
row_iter = flat_row_generator(JSON_PATH)
total_uploaded = 0
for chunk_num, rows in enumerate(chunked_iterable(row_iter, CHUNK_SIZE), start=1):
    ...
    print(f"Chunk {chunk_num} processed. Total uploaded: {total_uploaded}")

print("All done! Shipments uploaded in batches.")

"""
How to use:
1. Set SHIPMENTS_JSON, DB_URL (or DATABASE_URL), SHIPMENTS_TABLE, and optionally CHUNK_SIZE in your .env or edit above.
2. Run: python vintrick-backend/tools/upload_shipments_powerbi_style.py
3. Data will upload to the SQL table (auto-creates columns if needed), processing in memory-friendly batches.
"""